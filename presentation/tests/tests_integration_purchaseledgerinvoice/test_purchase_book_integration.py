...
Cubre el formulario (PurchaseLedgerInvoiceForm) y el ciclo de vida completo 
de las peticiones HTTP (Create, Update, Delete) validando el enrutamiento,
middlewares, vistas, flujos felices y el manejo defensivo de excepciones.
"""
...
def test_create_view_injects_tenant_context(
    logged_client, fiscal_profile, fiscal_period, base_invoice_data
):
    """[ID_HP_001] Verifica el ciclo completo y la inyección del contexto multi-tenant."""
    # Act
    response = logged_client.post("/purchase-invoices/new/", data=base_invoice_data)

    # Assert
    assert response.status_code == 302  # Redirección exitosa tras crear
    invoice = PurchaseLedgerInvoice.objects.get(number="INV-100")
    assert invoice.fiscal_profile == fiscal_profile
    assert invoice.fiscal_period == fiscal_period.start_period
...
def test_create_view_intercepts_validation_error(
    ...
    """[ID_EC_001] Valida intercepción de inconsistencias aritméticas del modelo."""
    # Arrange
    invalid_data = base_invoice_data.copy()
    invalid_data["vat_amount"] = "999.00"  # Monto intencionalmente incorrecto
    
    # Demostración explícita de fallo a nivel de modelo
    form = PurchaseLedgerInvoiceForm(data=invalid_data)
    form.instance.fiscal_profile = fiscal_profile
    form.instance.fiscal_period = fiscal_period.start_period
    form.is_valid()
    with pytest.raises(ValidationError):
        form.instance.clean()

    # Act
    response = logged_client.post("/purchase-invoices/new/", data=invalid_data)

    # Assert
    assert response.status_code == 200  # Se intercepta y retorna formulario no válido
    assert "El monto de IVA ingresado" in str(response.context["form"].errors)


@pytest.mark.django_db
def test_create_view_intercepts_integrity_error(
    logged_client, fiscal_profile, fiscal_period, base_invoice_data, supplier
):
    ...
    PurchaseLedgerInvoice.objects.create(
        ...
        fiscal_period=fiscal_period.start_period,
        supplier=supplier,
        number="INV-100",
...
def test_update_view_intercepts_fiscal_immutability_error(
    ...
    # Act
    response = logged_client.post(f"/purchase-invoices/{invoice.pk}/edit/", data=base_invoice_data)

    # Assert
    assert response.status_code == 200
    assert "Bloqueo de Modificación Fiscal" in str(response.context["form"].errors)


@pytest.mark.django_db
def test_delete_view_intercepts_fiscal_deletion_lock(
    logged_client, fiscal_profile, fiscal_period, supplier
):
    """[ID_EC_004] Comprueba que la petición de eliminación intercepta el bloqueo fiscal."""
    # Arrange
    invoice = PurchaseLedgerInvoice.objects.create(
        fiscal_profile=fiscal_profile,
        fiscal_period=fiscal_period.start_period,
    ...
    # Act
    response = logged_client.post(f"/purchase-invoices/{invoice.pk}/delete/")

    # Assert
    assert response.status_code == 200
    assert "Bloqueo de Eliminación Fiscal" in str(response.context["form"].errors)
...
def test_delete_view_intercepts_referential_integrity_error(
    ...
    # Act
    response = logged_client.post(f"/purchase-invoices/{parent_invoice.pk}/delete/")

    # Assert
    assert response.status_code == 200
    assert "No se puede eliminar el registro por integridad referencial" in str(response.context["form"].errors)